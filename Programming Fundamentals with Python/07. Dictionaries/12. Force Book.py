...
def force_book():
    ...
    while True:
        ...
        if '|' in command:
            ...
            side = command[0]
            member = command[1]
            create_force_side(side, member, users_info_dict)

        elif '->' in command:
            command = command.split(' -> ')
            member = command[0]
            side = command[1]
            create_force_user(member, side, users_info_dict)

    for data in users_info_dict:
        print(f'Side: {data}, Members: {len(users_info_dict[data])}')
        for name in users_info_dict[data]:
            print(f'! {name}')
